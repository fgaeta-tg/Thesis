...
the incrementCounter method(?) to let Datadog understand that every time that events verifies the counter should increase by one, so that the new value can be displayed in the dashboards, because Datadog works with this incrementCounter to activate a sort of connection with the metrics. Creating a metric directly on Datadog was a more complex process and not always possible (?), this was the fastest and most convenient way. 
