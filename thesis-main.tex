...
This is needed because any kind of discomfort that the client might experience while using the CRM Mywellness application is perceived as an interface/application error, even when it is not a responsibility of frontend programmers, but the common user has no technical expertise to understand it, hence their feedbacks will sound generic, too. Without this proper traceability methodology, we would waste a lot of time looking for the source of the problem almost randomly checking code files. 

\acp{CWV} is a set of metrics that measure real-world user experience for loading performance, interactivity, and visual stability of the page \cite{CWV}. The categories we are interested in refer mainly to the user experience impact: 
\begin{itemize}
